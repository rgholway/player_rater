import React, { Component } from 'react'

class Champions extends Component {
  constructor(props) {
    super(props)
    this.state = {player: ""}
  }

  render() {
    return(
      <div className="gray">
        <div className="first__16__line"></div>
        <div className="first__name">
          <div className="team__name">Roma</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/17/items/badges/6000084.png"/></div>
        </div>
        <div className="ninth__16__line"></div>
        <div className="ninth__name">
          <div className="team__name">Lyon</div>
          <div className="score">1</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/17/items/badges/6000100.png"/></div>
        </div>
        <div className="second__16__line"></div>
        <div className="second__name">
          <div className="team__name">FC Porto</div>
          <div className="score">4</div>
          <div className="badge"><img className= "photo_champions" src= "http://futhead.cursecdn.com/static/img/17/clubs/236.png"/></div>
        </div>
        <div className="tenth__16__line"></div>
        <div className="tenth__name">
          <div className="team__name">Barcelona</div>
          <div className="score">5</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/241.png"/></div>
        </div>
        <div className="third__16__line"></div>
        <div className="third__name">
          <div className="team__name">Man United</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000069.png"/></div>
        </div>
        <div className="fourth__16__line"></div>
        <div className="fourth__name">
          <div className="team__name">PSG</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/15/clubs_large/73.png"/></div>
        </div>
        <div className="fifth__16__line"></div>
        <div className="fifth__name">
          <div className="team__name">Ajax</div>
          <div className="score">5</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/15/clubs_large/245.png"/></div>
        </div>
        <div className="sixth__16__line"></div>
        <div className="sixth__name">
          <div className="team__name">Real Madrid</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000063.png"/></div>
        </div>
        <div className="seventh__16__line"></div>
        <div className="seventh__name">
          <div className="team__name">Tottenham</div>
          <div className="score">4</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000082.png"/></div>
        </div>
        <div className="eigth__16__line"></div>
        <div className="eigth__name">
          <div className="team__name">Dortmund</div>
          <div className="score">0</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000080.png"/></div>
        </div>
        <div className="eleventh__16__line"></div>
        <div className="eleventh__name">
          <div className="team__name">Liverpool</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/9.png"/></div>
        </div>
        <div className="twelfth__16__line"></div>
        <div className="twelfth__name">
          <div className="team__name">Bayern Munich</div>
          <div className="score">0</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/17/items/badges/6000061.png"/></div>
        </div>
        <div className="thirteenth__16__line"></div>
        <div className="thirteenth__name">
          <div className="team__name">Atletico Madrid</div>
          <div className="score">2</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/17/items/badges/6000075.png"/></div>
        </div>
        <div className="fourteenth__16__line"></div>
        <div className="fourteenth__name">
          <div className="team__name">Juventus</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src="https://www.moddingway.com/downloads/9b1f353621947f3f6171b8163cb8757d/4881/222673_logo1.png"/></div>
        </div>
        <div className="fifteenth__16__line"></div>
        <div className="fifteenth__name">
          <div className="team__name">Man City</div>
          <div className="score__black">10</div>
          <div className="badge"><img className= "photo_champions" src="http://futhead.cursecdn.com/static/img/17/clubs/10.png"/></div>
        </div>
        <div className="sixteenth__16__line"></div>
        <div className="sixteenth__name">
          <div className="team__name">Schalke 04</div>
          <div className="score">2</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/17/items/badges/6000097.png"/></div>
        </div>
        <div className="first__quarter__line"></div>
        <div className="first__quarter__name">
          <div className="team__name">Ajax</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/15/clubs_large/245.png"/></div>
        </div>
        <div className="second__quarter__line"></div>
        <div className="second__quarter__name">
          <div className="team__name">Juventus</div>
          <div className="score">2</div>
          <div className="badge"><img className= "photo_champions" src="https://www.moddingway.com/downloads/9b1f353621947f3f6171b8163cb8757d/4881/222673_logo1.png"/></div>
        </div>
        <div className="third__quarter__line"></div>
        <div className="third__quarter__name">
          <div className="team__name">Man City</div>
          <div className="score__black">4</div>
          <div className="badge"><img className= "photo_champions" src="http://futhead.cursecdn.com/static/img/17/clubs/10.png"/></div>
        </div>
        <div className="fourth__quarter__line"></div>
        <div className="fourth__quarter__name">
          <div className="team__name">Tottenham</div>
          <div className="score">4</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000082.png"/></div>
        </div>
        <div className="fifth__quarter__line"></div>
        <div className="fifth__quarter__name">
          <div className="team__name">Barcelona</div>
          <div className="score">4</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/241.png"/></div>
        </div>
        <div className="sixth__quarter__line"></div>
        <div className="sixth__quarter__name">
          <div className="team__name">Man Utd</div>
          <div className="score">0</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000069.png"/></div>
        </div>
        <div className="seventh__quarter__line"></div>
        <div className="seventh__quarter__name">
          <div className="team__name">Liverpool</div>
          <div className="score">6</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/9.png"/></div>
        </div>
        <div className="eigth__quarter__line"></div>
        <div className="eigth__quarter__name">
          <div className="team__name">FC Porto</div>
          <div className="score">1</div>
          <div className="badge"><img className= "photo_champions" src= "http://futhead.cursecdn.com/static/img/17/clubs/236.png"/></div>
        </div>
        <div className="first__semi__line"></div>
        <div className="first__semi__name">
          <div className="team__name">Ajax</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/15/clubs_large/245.png"/></div>
        </div>
        <div className="second__semi__line"></div>
        <div className="second__semi__name">
          <div className="team__name">Tottenham</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000082.png"/></div>
        </div>
        <div className="third__semi__line"></div>
        <div className="third__semi__name">
          <div className="team__name">Barcelona</div>
          <div className="score">3</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/241.png"/></div>
        </div>
        <div className="fourth__semi__line"></div>
        <div className="fourth__semi__name">
          <div className="team__name">Liverpool</div>
          <div className="score">4</div>
          <div className="badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/9.png"/></div>
        </div>
        <div className="final">
          <div className="final__title">Final</div>
          <div className="final__second">
            <div className="final__name">Liverpool</div>
            <div className="final__score__second">2</div>
            <div className="final__badge__second"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/9.png"/></div>
          </div>
          <div className="final__first">
            <div className="final__name">Tottenham</div>
            <div className="final__score">0</div>
            <div className="final__badge"><img className= "photo_champions" src= "https://futhead.cursecdn.com/static/img/17/items/badges/6000082.png"/></div>
          </div>
          <div className="final__champion">Champion:</div>
          <div className="final__team">
            <div className="champion__name">Liverpool</div>
            <div className="champion__badge"><img className= "photo_champions" src="https://futhead.cursecdn.com/static/img/15/clubs_large/9.png"/></div>
          </div>
        </div>
        </div>
    )}
  }

  export default Champions
